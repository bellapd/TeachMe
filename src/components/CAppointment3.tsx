...
function Appointment() {
  ...
  const updateMetadata = async () => {
    ...
    if (cour === undefined) {
      const data = { courses: { "c": [[getFormattedDate(), selectedTime, "Chika", "c"]] } };
          ...
          <div className="flex-col items-center px-4 md:px-80 mt-5">
            ...
            >
              <Button onClick={() => {
                // sendEmail();
                updateMetadata()
              }} variant="text" className="rounded-full px-10 py-4 mb-5 text-center text-[#4700C6] bg-[#FFE873] hover:bg-[#4700C6] hover:text-[#FFE873] hover:scale-105 transform transition duration-300 text-sm">
                Make an Appointment
            </Button>
            </Link>
          </div>
